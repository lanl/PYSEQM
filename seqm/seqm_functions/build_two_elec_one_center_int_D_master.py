...
from .two_elec_two_center_int import GetSlaterCondonParameter, GetSlaterCondonParameter_test
from .cal_par import *
...
def calc_integral(zetas, zetap, zetad, Z, size, maskd, P0, F0SD, G2SD):
        ...
        for k in zetad:
            ...
            if(zetap[j] > 0 and zetad[j] > 0 ):
                ...
                CCCC  =GetSlaterCondonParameter_test(1,int(psindex[j]),zetas[j],int(psindex[j]),zetap[j],int(psindex[j]),zetap[j],int(dindex[j]),zetad[j])
        ...
        #print('N E\n', int(psindex),zetas,int(psindex),zetap,int(psindex),zetap,int(dindex),zetad )
        ...
        W = torch.zeros(size,243, device=Z.device)
        IntRf1 = [ \
...
def calc_integral_os(zetas, zetap, zetad, Z, size, maskd, P0, F0SD, G2SD):
        const = Constants().to(Z.device)
        #### populations elements with d-oribitals
        isY = (zetad != 0)
        j = 0
        ...
        #print('N E\n', psindex,zetas,psindex,zetas,dindex,zetad,dindex,zetad)
        #print( R016, R036, R066, R155, R125, R244, R236, R266, R234, R246, R355, R466)
        half = 0.5
        ONE = 1.0
        TWO = 2.0
        THREE = 3.0
...
#                print(W[maskd,j],i1,i2,IntRep[j]-1)
            if(i1>0):
                    W_exch[maskd,j] += 0.25*integral[...,i1-1]
#                    if(j == 203):
#                        print(W[maskd,j],i1)
            if(i2>0):
                    W_exch[maskd,j] += 0.25*integral[...,i2-1]
#                    if(j == 203):
#                        print(W[maskd,j],i2)
