...
def overlap_full(const, nmol, molsize, maskd, mask, mask_l, idxi, idxj, ni, nj, xij, rij, Z, zetas, zetap):
    """
    ...
    di[overlap_pairs] = diatom_overlap_matrix(
        ni[overlap_pairs],
        nj[overlap_pairs],
        xij[overlap_pairs],
        rij[overlap_pairs],
    ...
    di_full = (
        di_full.reshape(nmol, molsize, molsize, 4, 4).transpose(2, 3).reshape(nmol, 4 * molsize, 4 * molsize)
    )
