...
class SCF(torch.autograd.Function):
    ...
    def backward(ctx, grad0, grad1):
        ...
        while(1):
            ...
            if k >= SCF_BACKWARD_MAX_ITER: break
...
#             grads[i] = torch.sum(torch.stack([gradients[l][t+1] for l in range(1, ln)]), dim=0)
        for i in gvind:
            for l in range(1, ln): grads[i].add_(gradients[l][t]) # IN-PLACE OPERATION TRACKED?
            t = t + 1
