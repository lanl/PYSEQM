...
def rpa(mol, w, e_mo, nroots, root_tol, init_amplitude_guess=None):
    ...
    if nroots > nov:
        raise Exception(
            f"Maximum number of roots for this molecule is {nov}. Reduce the requested number of roots"
        )
    ...
    while davidson_iter <= max_iter:  # Davidson loop
        ...
        amplitude_X, amplitude_Y, resid_norm, correction_directionR, correction_directionL = calc_rpa_residue(
            AV, BV, X, Y, V, vend_max, e_val_n
        )
