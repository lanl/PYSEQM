...
def scf_grad(P, molecule, const, method, mask, maskd, molsize, idxi,idxj, ni,nj,xij,rij, parnuc,
    ...
    for coord in range(3):
        ...
        gam_ = w_plus[:,0,0]
        EnucAB_plus = pair_nuclear_energy(Z,const, nmol, ni, nj, idxi, idxj, rij_,rho0xi,rho0xj,molecule.alp, molecule.chi, gam=gam_, method=method, parameters=parnuc)
        Xij[:, coord] += 2.0 * delta
        rij_ = torch.norm(Xij, dim=1)
        xij_ = Xij / rij_.unsqueeze(1)
        rij_ = rij_/a0
        w_minus, e1b_minus, e2a_minus, rho0xi,rho0xj, _, _ = TETCI(const, idxi, idxj, ni, nj, xij_, rij_, Z,\
                                        ...
                                        molecule.alp, molecule.chi, molecule.method)
        gam_ = w_minus[:,0,0]
        EnucAB_minus = pair_nuclear_energy(Z, const, nmol, ni, nj, idxi, idxj, rij_, rho0xi,rho0xj,molecule.alp, molecule.chi,gam=gam_, method=method, parameters=parnuc)
        Xij[:, coord] -= delta
...
def core_core_der(alpha,rij,Xij,ZAZB,ni,nj,idxi,idxj,gam,w_x,method,parameters):
    ...
    t5_der = torch.sum(K[idxi]*torch.exp(-L[idxi]*(rija[:,None]-M[idxi])**2)*L[idxi]*(rija[:,None]-M[idxi]),dim=1)
    t6_der = torch.sum(K[idxj]*torch.exp(-L[idxj]*(rija[:,None]-M[idxj])**2)*L[idxj]*(rija[:,None]-M[idxj]),dim=1)
                 ...
                 for nn in range(0,mm+1):
                     ...
                     if kk==0:
                         ...
                         if mm==0:
                             ...
                             wXH_x[...,idxXH] = riXH_x[...,1]*rotXH[:,None,0,k] + riXH[:,None,1]*rot_derXH[:,:,0,k]
                             idxXH = idxXH + 1
                         elif nn==0:
                             # (ps|ps)
