...
class Parser(torch.nn.Module):
    ...
    def __init__(self, seqm_parameters):
        ...
        if seqm_parameters.get("elements") is None:
            raise RuntimeError(
                "Please instantiate a Molecule object before instantiating an object of Electronic_structure or Molecular_Dynamics class."
    ...
    def forward(self, molecule, themethod, return_mask_l=False, do_large_tensors=True, *args, **kwargs):
        ...
        if themethod == "PM6":  # PM6 is not implemented yet
            nHeavy = torch.sum(
                (
                    ...
                    & (
                        (molecule.species <= 12)
                        | ((molecule.species >= 18) & (molecule.species <= 20))
                        | ((molecule.species >= 30) & (molecule.species <= 32))
                        | ((molecule.species >= 36) & (molecule.species <= 38))
                        | ((molecule.species >= 48) & (molecule.species <= 50))
                        | ((molecule.species >= 54) & (molecule.species <= 56))
                        | ((molecule.species >= 80) & (molecule.species <= 83))
                    )
                ),
                dim=1,
            )
        ...
        nSuperHeavy = torch.sum(
            (
                ((molecule.species > 12) & (molecule.species < 18))
                | ((molecule.species > 20) & (molecule.species < 30))
                | ((molecule.species > 32) & (molecule.species < 36))
                | ((molecule.species > 38) & (molecule.species < 48))
                | ((molecule.species > 50) & (molecule.species < 54))
                | ((molecule.species > 70) & (molecule.species < 80))
                | (molecule.species == 57)
            ),
            dim=1,
        )
        ...
        n_charge = (
            torch.sum(tore[molecule.species], dim=1).reshape(-1).type(torch.int64)
        )  # number of valence electrons, N*|e|
        if torch.is_tensor(molecule.tot_charge):
            n_charge -= molecule.tot_charge.reshape(-1).type(torch.int64)

        if self.uhf:
            nocc_alpha = n_charge / 2.0 + (molecule.mult - 1) / 2.0
            nocc_beta = n_charge / 2.0 - (molecule.mult - 1) / 2.0
...
class Hamiltonian(torch.nn.Module):
    ...
    def forward(self, molecule, themethod, P0=None):
        ...
        v : eigenvectors of F
        """
        F, e, P, Hcore, w, charge, rho0xi, rho0xj, riXH, ri, notconverged, molecular_orbitals = scf_loop(
            molecule,
            eps=self.eps,
            P=P0,
            sp2=self.sp2,
...
class Energy(torch.nn.Module):
    ...
    def forward(
        ...
        if excited_mask.any() and not self.excited_states and not self.xlesmd:
            raise Exception(
                "You have requested for excited state dynamics but have not given input parameters for excited states (like n_states) in seqm_parameters"
            )
        ...
        if self.excited_states:
            ...
            if self.seqm_parameters.get("do_all_forces", False):
                ...
                if not do_analytical_gradient[0]:
                    raise Exception(
                        "when asking for calculating all gradients of excited states ask for analytical gradients"
                    )
                if not all_same_mols:
                    raise NotImplementedError
                ...
                for i in range(1, nroots + 1):
                    ...
                    molecule.all_forces[:, i, ...] = -rcis_grad_batch(
                        molecule,
                        w,
                        e,
                        riXH,
        ...
        if all_terms:
            ...
            Eiso = elec_energy_isolated_atom(
                molecule.const,
                molecule.Z,
            ...
            Hf, Eiso_sum = heat_formation(
                molecule.const, molecule.nmol, molecule.atom_molid, molecule.Z, Etot, Eiso, flag=self.Hf_flag
            )
...
class Force(torch.nn.Module):
    ...
    def forward(
        ...
        Hf, Etot, Eelec, Enuc, Eiso, _, e_gap, e, D, charge, notconverged = self.energy(
            molecule,
        ...
        if self.seqm_parameters.get("normal modes", False):
            if self.seqm_parameters.get("scf_backward", 0) != 2:
                raise Exception(
                    "You have requested for normal mode calculation but scf_backward has not been set to 2 in your input seqm_parameters"
                )
