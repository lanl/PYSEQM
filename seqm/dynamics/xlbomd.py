...
class EnergyXL(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        Hcore = (
            M.reshape(molecule.nmol, molecule.molsize, molecule.molsize, 4, 4)
            .transpose(2, 3)
        ...
        if "max_rank" in xl_bomd_params:  # Krylov
            ...
            with torch.set_grad_enabled(
                ...
                dDS = K0 * (
                    D - P
                )  # tr2  #W0 = K0*(DS - X) from J. Chem. Theory Comput. 2020, 16, 6, 3628–3640, alg 3
                ...
                while k < Rank - 1 and torch.max(Error) > err_threshold:
                    ...
                    for j in range(0, k):  # Orthogonalized Krylov vectors (Arnoldi)
                        ...
                        V[:, :, :, k] = (
                            V[:, :, :, k]
                            - torch.sum(V[:, :, :, k].transpose(1, 2) * V[:, :, :, j], dim=(1, 2)).view(
                                -1, 1, 1
                            )
                            * V[:, :, :, j]
                        )
            ...
            with torch.set_grad_enabled(
                ...
                if sp2[0]:
                    ...
                    e, D, molecule.molecular_orbitals = sym_eig_trunc(
                        F, molecule.nHeavy, molecule.nHydro, molecule.nocc
                    )  # [0:2]
                    lumo = molecule.nocc.unsqueeze(0).T
                    e_gap = (e.gather(1, lumo) - e.gather(1, lumo - 1)).reshape(-1)
