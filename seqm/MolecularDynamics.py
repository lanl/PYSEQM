...
class Molecular_Dynamics_Basic(torch.nn.Module):
    ...
    def zero_com(self, const, species, coordinates, velocities):
        ...
        with torch.no_grad():
            ...
            # inertia of moment
            Inertia = torch.sum(mass * torch.norm(coordinates, dim=2, keepdim=True)**2, dim=1, keepdim=True) \
            ...
            # omega, _ = torch.solve(L.unsqueeze(2), Inertia)
            omega = torch.linalg.solve(Inertia, L.unsqueeze(2))
            ...
            del r_com, v_com, L, Inertia, omega, T0, T1, alpha  # , Lnew, Tnew
