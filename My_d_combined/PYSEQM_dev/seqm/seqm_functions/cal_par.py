...
def dd_qqdorbitals(qn,qd,Z, zs, zp, zd):
    ...
    for k in qn:
        ...
        i = 1 + i
    qq = math.sqrt(1/3)*qnfactorial1*(1/(torch.sqrt(qnfactorial*qnfactorial)))*torch.pow((2*zs),(qn+1/2))*torch.pow((2*zp),(qn+1/2))*torch.pow((zs+zp),(-qn-qn-2))
    dd = math.sqrt(1/5)*torch.sqrt(qnfactorial2*1/(torch.sqrt(qnfactorial*qnfactorial))*torch.pow((2*zp),(qn+1/2))*torch.pow((2*zp),(qn+1/2))*torch.pow((zp+zp),(-qn-qn-3)))
    dorbdorb = math.sqrt(1/7)*torch.sqrt(qnddfactorial2*1/(torch.sqrt(qndfactorial*qndfactorial))*torch.pow((2*zd),(qd+1/2))*torch.pow((2*zd),(qd+1/2))*torch.pow((zd+zd),(-qd-qd-3)))
    dorbsorb = math.sqrt(math.sqrt(1/15))*torch.sqrt(qndfactorial2*1/(torch.sqrt(qndfactorial*qnfactorial))*torch.pow((2*zd),(qd+1/2))*torch.pow((2*zs),(qn+1/2))*torch.pow((zs+zd),(-qn-qd-3)))
    dorbporb = math.sqrt(1/5)*qndfactorial1*(1/(torch.sqrt(qnfactorial*qndfactorial)))*torch.pow((2*zp),(qn+1/2))*torch.pow((2*zd),(qd+1/2))*torch.pow((zp+zd),(-qd-qn-2))
      ...
      for i in FG:
          ...
          while ( I < 100):
              ...
              if(L == 1):
                  ...
                  F1  = (EV8*(1.0000/Y1-2.00000/math.sqrt(Y1**2+dsq*0.50000) +1.00000/math.sqrt(Y1**2+dsq)) - fg)**2
                  F2  = (EV8*(1.0000/Y2-2.0000/math.sqrt(Y2**2+dsq*0.50000)+1.0000/math.sqrt(Y2**2+dsq)) - fg)**2
              if(F1 < F2):
                  A2  = Y2
              else:
                  A1  = Y1
...
def AIJL(Z1,Z2,N1,N2,L):
      ...
      if(Z1 == 0 or Z2 == 0):
              ...
              AIJL = AIJL*( 2*Z1/(Z1+Z2) )**N1 * math.sqrt( 2*Z1/(Z1+Z2) ) *( 2*Z2/(Z1+Z2) )**N2 * math.sqrt( 2*Z2/(Z1+Z2) )/(Z1+Z2)**L
