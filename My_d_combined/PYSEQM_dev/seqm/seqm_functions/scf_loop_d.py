...
from .fock_u_batch_d import fock_u_batch_d
...
from .pack import *
from .packd import *
from .diag_d import sym_eig_trunc, sym_eig_trunc1, sym_eig_truncd, sym_eig_trunc1d
import warnings
import time
from .build_two_elec_one_center_int import calc_integral, calc_integral_os
...
def scf_forward0_u(M, w, W, gss, gpp, gsp, gp2, hsp, \
    ...
    err = torch.ones(nmol, dtype=P.dtype, device=P.device)
    notconverged = torch.ones(nmol,dtype=torch.bool, device=M.device)
    F = fock_u_batch_d(nmol, molsize, P, M, maskd, mask, idxi, idxj, w, W, gss, gpp, gsp, gp2, hsp,themethod, zetas, zetap, zetad, Z, F0SD, G2SD)
    if(themethod == 'PM6'):
        Hcore = M.reshape(nmol,molsize,molsize,9,9) \
                 .transpose(2,3) \
                 .reshape(nmol, 9*molsize, 9*molsize)
    ...
    while(1):
        ...
        if notconverged.any():
            if backward:
                if(themethod == 'PM6'):
                    e, P_ab[notconverged], v = sym_eig_trunc1d(F[notconverged],
                                                   nSuperHeavy[notconverged],
                                                   nHeavy[notconverged],
                                                   nHydro[notconverged],
                                                   nOccMO[notconverged])
                ...
                """
                if(themethod == 'PM6'):
                    e, P_ab[notconverged], v = sym_eig_truncd(F[notconverged],
                                                   nSuperHeavy[notconverged],
                                                   nHeavy[notconverged],
                                                   nHydro[notconverged],
                                                   nOccMO[notconverged])
    ...
    *   TIMES [F*P] FOR ITERATION J.
    ...
    for i in range(nDirect1):
        ...
        if notconverged.any():
            ...
            notconverged =  err>eps
            if debug:
                print("scf nDirect1", k, torch.max(err).item(), torch.sum(notconverged).item(),  time.time()-start_time )
            k+=1
        else:
            return P, notconverged
    ...
    for i in range(nAdapt):
        ...
        if notconverged.any():
            ...
            notconverged =  err>eps
            if debug:
                print("scf nAdapt  ", k, torch.max(err).item(), torch.sum(notconverged).item(),  time.time()-start_time )
            k+=1
        else:
            return P, notconverged
    #
    ...
    while (cFock<2):
        ...
        if notconverged.any():
            ...
            notconverged =  err>eps
            if debug:
                print("scf cFock   ", k, torch.max(err).item(), torch.sum(notconverged).item(),  time.time()-start_time )
            k+=1
        else:
            return P, notconverged
    ...
    while(1):
        ...
        if notconverged.any():
            ...
            if debug:
                print("scf pulay   ", k, torch.max(err).item(), torch.sum(notconverged).item(),  time.time()-start_time )
            k+=1
            if k >= MAX_ITER:
                return P, notconverged
        else:
...
class SCF(torch.autograd.Function):
    ...
    def backward(ctx, grad0, grad1):
        ...
        # if(torch.sum(torch.abs(zetad)) > 0):
        #     molsize = Pin.shape[1]//9
        #     themethod = 'PM6'
        # else:
        #     molsize = Pin.shape[1]//4
        ...
        with torch.enable_grad():
            ...
            F = fock(nmol, molsize, Pin, M, maskd, mask, idxi, idxj, w, W, gss, gpp, gsp, gp2, hsp, themethod, zetas, zetap, zetad, Z, F0SD, G2SD)
            """
...
def scf_loop(const, molsize, \
            ...
            zetas, zetap, zetad, zs, zp, zd, uss, upp , udd,  gss, gsp, gpp, gp2, hsp, F0SD, G2SD, rho_core, alpha, chi, themethod, beta, Kbeta=None, \
    ...
    #W = calc_integral(zetas, zetap, zetad, Z, nmol*molsize*molsize, maskd, P, F0SD, G2SD)
    #"""
    ...
    if scf_backward==2:
        ...
        elif scf_converger[0] == 1: # $$$ not sure if converger 1 works. tests needed.
        ...
        elif scf_converger[0] == 2: # $$$ not sure if converger 1 works. tests needed.
            Pconv, notconverged =  scf_forward2(  M, w, W, gss, gpp, gsp, gp2, hsp, \
                         nHydro, nHeavy, nSuperHeavy, nOccMO, \
                         nmol, molsize, \
                         maskd, mask, idxi, idxj, P, eps, themethod, zs, zp, zd, Z, F0SD, G2SD, sp2=sp2, backward=True)
    ...
    #print('full scf time', time.time()-t0)
    #"""
    if notconverged.any():
    ...
    if Pconv.dim()==4:
        F = fock_u_batch_d(nmol, molsize, Pconv, M, maskd, mask, idxi, idxj, w, W, gss, gpp, gsp, gp2, hsp, themethod, zs, zp, zd, Z, F0SD, G2SD)
    else:
    ...
    if eig:
        ...
        if(themethod == 'PM6'):
            if scf_backward>=1:
                e, v = sym_eig_trunc1d(F, nSuperHeavy, nHeavy, nHydro, nOccMO,eig_only=True)
            else:
                e, v = sym_eig_truncd(F, nSuperHeavy, nHeavy, nHydro, nOccMO,eig_only=True)


            #get charge of each orbital on each atom
            ### not sure if these charge calculations are correct. $$$
            #####
            ##### CHARGES on orbitals ARE WRONG! Differ in UHF and RHF.
            
            
            charge = torch.zeros(nmol,molsize*9, molsize, device=e.device, dtype=e.dtype)
            v2 = [x**2 for x in v]
            norb = 4 * nHeavy + nHydro + 9*nSuperHeavy
            if F.dim() == 4: # open shell
                for i in range(nmol):
                    charge[i,:norb[i],:nSuperHeavy[i]] =  v2[i][0,:norb[i],:(9*nSuperHeavy[i])].reshape(norb[i],9,nSuperHeavy[i]).sum(dim=1)
                    charge[i,:norb[i],:nSuperHeavy[i]] += v2[i][1,:norb[i],:(9*nSuperHeavy[i])].reshape(norb[i],9,nSuperHeavy[i]).sum(dim=1)
                    
                    charge[i,:norb[i],nSuperHeavy[i]:(nHeavy[i]+nSuperHeavy[i])] =  v2[i][0,:norb[i],(9*nSuperHeavy[i]):(4*nHeavy[i]+9*nSuperHeavy[i])].reshape(norb[i],4,nHeavy[i]).sum(dim=1)
                    charge[i,:norb[i],nSuperHeavy[i]:(nHeavy[i]+nSuperHeavy[i])] += v2[i][1,:norb[i],(9*nSuperHeavy[i]):(4*nHeavy[i]+9*nSuperHeavy[i])].reshape(norb[i],4,nHeavy[i]).sum(dim=1)
                    
                    charge[i,:norb[i],(nSuperHeavy[i]+nHeavy[i]):(nSuperHeavy[i]+nHeavy[i]+nHydro[i])] =  v2[i][0,:norb[i],(4*nHeavy[i]+9*nSuperHeavy[i]):(4*nHeavy[i]+nHydro[i]+9*nSuperHeavy[i])]
                    charge[i,:norb[i],(nSuperHeavy[i]+nHeavy[i]):(nSuperHeavy[i]+nHeavy[i]+nHydro[i])] += v2[i][1,:norb[i],(4*nHeavy[i]+9*nSuperHeavy[i]):(4*nHeavy[i]+nHydro[i]+9*nSuperHeavy[i])]
                    
                    charge = charge/2
                    
            else: # closed shell
                for i in range(nmol):
                    charge[i,:norb[i],:nSuperHeavy[i]] = v2[i][:norb[i],:(9*nSuperHeavy[i])].reshape(norb[i],9,nSuperHeavy[i]).sum(dim=1)
                    charge[i,:norb[i],nSuperHeavy[i]:(nHeavy[i]+nSuperHeavy[i])] = v2[i][:norb[i],(9*nSuperHeavy[i]):(4*nHeavy[i]+9*nSuperHeavy[i])].reshape(norb[i],4,nHeavy[i]).sum(dim=1)
                    charge[i,:norb[i],(nSuperHeavy[i]+nHeavy[i]):(nSuperHeavy[i]+nHeavy[i]+nHydro[i])] = v2[i][:norb[i],(4*nHeavy[i]+9*nSuperHeavy[i]):(4*nHeavy[i]+nHydro[i]+9*nSuperHeavy[i])]
