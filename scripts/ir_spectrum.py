...
def unit_vel_to_au(units):
    ...
    if u == "bohr/au":
        return 1.0
    if u == "a/fs":
        return ang_per_fs_to_au
    if u == "a/ps":
        return ang_per_fs_to_au * 1e-3
    if u == "m/s":
        return ang_per_fs_to_au * 1e-5
    raise ValueError(f"Unknown --vel-units: {units}")


def unit_dip_to_au(units):
    u = units.lower()
    if u in ("au", "a.u.", "atomic"):
        return 1.0
    if u in ("debye", "d"):
        return 1.0 / debye_per_au
    if u in ("c*m", "c·m", "c m"):
        return 1.0 / ea0
...
def dt_to_au(dt_value, dt_units):
    ...
    if u == "fs":
        return float(dt_value) / au_to_fs
...
def vdos_from_vel(
    V, dt, masses, window, exp_width_cm, zp_factor, min_wn, max_wn, mass_weight, remove_com_flag
):
    if remove_com_flag:
    ...
    win = (
        np.hanning(T)
        if window == "hann"
        else (exponential_window(T, dt, exp_width_cm) if window == "exp" else np.ones(T))
    )
    ENBW = window_normalization(win)
    wV = V * win[:, None, None]
    if mass_weight:
...
def main():
    ap = argparse.ArgumentParser(
        description="Compute IR (from dipoles) and/or VDOS (from velocities) and optionally plot.",
    ...
    ap.add_argument(
        ...
        choices=["auto", "yes", "no"],
        default="auto",
        help="Whether dipole file has a time column",
    )
    ...
    ap.add_argument(
        "--vel-cols",
        default="-3,-2,-1",
        help="Indices of vx,vy,vz in each atom line (comma-separated, Python-style)",
    )
    ...
    ap.add_argument(
        "--plot", nargs="?", const="show", help='Plot spectrum(s): "show" (default) or filename.png/.pdf'
    )
    ...
    if args.dip:
        ...
        wn, I = ir_from_dipoles(
            M, dt_au, args.use_deriv, args.window, args.exp_width, args.zp, args.min_wn, args.max_wn
        )
