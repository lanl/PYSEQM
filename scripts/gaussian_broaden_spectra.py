...
def main():
    p = argparse.ArgumentParser(
        description="Plot Gaussian-broadened absorption/emission spectra on one figure."
    )
    io = p.add_argument_group("Inputs")
    io.add_argument(
        "--abs", dest="absfile", help="Absorption input .dat (two columns: energy[eV], intensity)"
    )
    ...
    widths.add_argument(
        "--fwhm", type=float, help="Global FWHM for both curves (overridden by per-curve flags)"
    )
    widths.add_argument(
        "--sigma", type=float, help="Global σ for both curves (overridden by per-curve flags)"
    )
    ...
    grid.add_argument(
        "--xmin", type=float, help="Min energy for grid in eV (default: min(data)-1.5*max(FWHM))"
    )
    grid.add_argument(
        "--xmax", type=float, help="Max energy for grid in eV (default: max(data)+1.5*max(FWHM))"
    )
    ...
    grid.add_argument("--no-normalize", dest="normalize", action="store_false", help="Disable normalization.")
    ...
    try:
        ...
        if args.absfile:
            sigma_abs, width_lbl_abs = _sigma_from_args(
                args.sigma, args.fwhm, args.sigma_abs, args.fwhm_abs, "absorption"
            )
        if args.emfile:
            sigma_em, width_lbl_em = _sigma_from_args(
                args.sigma, args.fwhm, args.sigma_em, args.fwhm_em, "emission"
            )
        ...
        if args.dx is None:
            ...
            if args.emfile and xs_em.size > 1:
                ...
                if diffs.size:
                    native_dx_candidates.append(np.median(diffs))
            native_dx = (
                min(native_dx_candidates)
                if native_dx_candidates
                else (max(widths_for_pad) / 50.0 if widths_for_pad else 0.002)
            )
    ...
    if args.axis == "eV":
        ...
        if args.absfile:
            o = np.argsort(sticks_abs_x)
            sticks_abs_x, sticks_abs_y = sticks_abs_x[o], sticks_abs_y[o]
        if args.emfile:
            o = np.argsort(sticks_em_x)
            sticks_em_x, sticks_em_y = sticks_em_x[o], sticks_em_y[o]
    ...
    if args.out_csv:
        ...
        if Y_abs_plot is not None:
            cols.append(Y_abs_plot)
            hdrs.append("broadened_abs")
        if Y_em_plot is not None:
            cols.append(Y_em_plot)
            hdrs.append("broadened_em")
    ...
    if Y_abs_plot is not None:
        plt.plot(Xplot, Y_abs_plot, linewidth=1.8, ls="-", label="Absorption")  # ({width_lbl_abs})")
    if Y_em_plot is not None:
        plt.plot(Xplot, Y_em_plot, linewidth=2.0, ls="-.", label="Emission")  # ({width_lbl_em})")
